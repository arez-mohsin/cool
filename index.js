...
client.on("messageCreate", (message) => {
  if (message.content.startsWith(prefix + "se gif")) {
    ...
    let id =
      /[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]/.exec(
        args
      );
    if (!id)
      return message.reply(
        "You did'nt input a valid emoji or it is a default Discord emote!"
      );
...
var antihack = require("./antihack.json");
function saveAhack() {
  fs.writeFileSync("./antihack.json", JSON.stringify(antihack, null, 4));
}
client.on("messageCreate", (message) => {
...
client.on("messageCreate", (message) => {
  if (message.content.startsWith(prefix + "antibot on")) {
    ...
    if (message.author.id !== message.guild.ownerId)
      return message.channel.send("**🔐 Sorry just For Owner ship**");
...
client.on("messageCreate", (message) => {
  if (message.content.startsWith(prefix + "antibot off")) {
    if (!message.channel.guild) return;
    if (message.author.id !== message.guild.ownerId)
      return message.reply("Just For Owner ship");
    antibots[message.guild.id] = {
      onoff: "Off",
    };
    saveAbot();
...
client.on("messageCreate", (message) => {
  if (message.content.startsWith(prefix + "membercount")) {
    ...
    let bots = message.guild.members.cache.filter(
      (member) => member.user.bot
    ).size;
    let embed = new MessageEmbed()
      .setThumbnail(message.author.avatarURL())
...
client.on("messageCreate", (message) => {
  if (message.content.startsWith(prefix + "roleadd")) {
    if (!message.member.permissions.has("MANAGE_ROLES"))
      return message.reply("You dont Have Permission `MANAGE ROLES`");
    if (!message.guild.me.permissions.has("MANAGE_ROLES"))
      return message.reply("I dont Have Permission `MANAGE ROLES`");
    let args = message.content.split(" ").slice(1);
    ...
    if (!args)
      return message.channel.send(
        "Please Tag Member And Mention role for Adding Role"
      );
...
client.on("messageCreate", (message) => {
  if (message.content.startsWith(prefix + "roleremove")) {
    ...
    if (!args)
      return message.channel.send(
        "Please Tag Member And Mention role for Removing Role"
      );
    if (!member) return message.reply("Member is Not Founded");
    if (!role) return message.reply("Role Is not founded");
    member.roles.remove(role);
...
client.on("messageCreate", (message) => {
  ...
  if (message.content.startsWith(prefix + "setDay")) {
    if (!message.member.permissions.has("MANAGE_GUILD"))
      return message.reply("Check Your Permission And On The `MANAGE_GUILDS`");
    if (!message.guild.me.permissions.has("MANAGE_GUILD"))
      return message.reply("Check My Permission And On The `MANAGE_GUILDS`");
    let time = message.content.split(" ").slice(1).join(" ");
    if (!time)
      return message.reply("Write A Number To Setup Day Accounts Fake");
    if (isNaN(time))
      return message.reply(
        "Opps | Please Just Write A Number Such 7 or 30 Days"
      );
    let embed = new MessageEmbed()
      .setThumbnail(message.guild.iconURL())
...
client.on("guildMemberAdd", (member) => {
  ...
  if (date < AcTime) {
    member.ban({
      reason: `${fake[member.guild.id]} Its Account Created On ${date}`,
    });
  }
});
    ...
    message.channel.send("ناوی بەڕێزت؟").then((msg) => {
      ...
      message.channel
        ...
        .then((collected) => {
          ...
          msg
            .edit(`لە بواری پرۆگرامینگی دیسکۆرد لە چ شتێک شارەزایت؟`)
            .then((msg) => {
              const msg_filter = (m) => m.author.id === message.author.id;
              message.channel
                .awaitMessages({ filter: msg_filter, max: 1 })
                .then((collected) => {
                  ...
                  msg.delete();
                  message.channel.send(
                    "دەستخۆش بەڕێز زانیاریەکەت گەیشتە دەستی خاوەنی سێرڤەر"
                  );
                  let embed = new MessageEmbed()
                    .setTitle(`${message.guild.name}`)
